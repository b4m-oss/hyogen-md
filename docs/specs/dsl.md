...
| 関数呼び出し | **ユーザー登録関数のみ**（現状は `component ... as name` で登録した `name(...)`）。将来 hyogen.md ビルトイン関数を追加する可能性あり |
| メソッド呼び出し | 当面 **`.toLocaleString(...)` のみ許容**。それ以外のメソッドは不可 |
...
- 許可メソッドの追加（都度 spec 更新。当面 `.toLocaleString` のみ）
- 初期リリース時点のビルトイン関数（実装が出てから検討）
- mixin（保留。必要時に再検討）
